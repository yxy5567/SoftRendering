...
//被遮挡物体模型空间到世界空间的变换阵
Matrix4D_float m_MDLtoWLD_Ground;
//世界空间到ClipSpace中的变换阵
...
//地板顶点的世界坐标
Vector4D_float* vertCoord_worldSpace_Ground, *vertCoord_Screenf_Ground, *vertCoord_clipSpace_Ground;
//网状体对象
Mesh m1,m2;
//互斥锁
std::mutex mtx1, mtx2;
...
//camera shift
float camera_Zshift = 0, camera_Yshift = 0, camera_X_shift = 0;
...
//	mat1.MatrixScale(cubeScale, cubeScale, cubeScale);
	mat1.MatrixScale(cubeScale, cubeScale_y*cubeScale, cubeScale);
...
void coordTrans_Ground(Mesh& m, Matrix4D_float& mat_Trans)
{
	...
	m3.MatrixRotate_Y(rotate_Y_Angle);
	mat1.MatrixScale(cubeScale, cubeScale, cubeScale);
	mat2.MatrixShift(0, -0.3, 2);
	//	mat2.MatrixShift(-1, 0, 2.5);
	mat_MDLtoWLD = mat2*mat1*m3*m4*m5;

	//将从模型坐标系到世界坐标系的变换矩阵存储出来，用于计算漫反射
	m_MDLtoWLD_Ground = mat_MDLtoWLD;
...
{
	//初始化
	if (isClearZBuffer) clearZbuffer();
...
//-----------------------------------------------------------下面是用于Shadow Map阴影相关----------------------------------------------------------------------
int testcountgerworld = 0;
//将指定地板Mesh的指定光栅上的指定屏幕坐标的点得到其对应的世界坐标
Vector4D_float getCoordWorld_Ground(const Mesh& mesh, int surfaceIndex, int faceIndex, float coordX_screen, float coordY_screen)
{
	float m, n;
	int vertIndex_A = mesh.m_surface[surfaceIndex].m_face[faceIndex].m_vert[0].m_vertIndex;
	int vertIndex_B = mesh.m_surface[surfaceIndex].m_face[faceIndex].m_vert[1].m_vertIndex;
	int vertIndex_C = mesh.m_surface[surfaceIndex].m_face[faceIndex].m_vert[2].m_vertIndex;

	float X_clip_A = vertCoord_clipSpace_Ground[vertIndex_A].get_X();
	float X_clip_B = vertCoord_clipSpace_Ground[vertIndex_B].get_X();
	float X_clip_C = vertCoord_clipSpace_Ground[vertIndex_C].get_X();
	float Y_clip_A = vertCoord_clipSpace_Ground[vertIndex_A].get_Y();
	float Y_clip_B = vertCoord_clipSpace_Ground[vertIndex_B].get_Y();
	float Y_clip_C = vertCoord_clipSpace_Ground[vertIndex_C].get_Y();
	float W_clip_A = vertCoord_clipSpace_Ground[vertIndex_A].get_W();
	float W_clip_B = vertCoord_clipSpace_Ground[vertIndex_B].get_W();
	float W_clip_C = vertCoord_clipSpace_Ground[vertIndex_C].get_W();
	...
	n = 1.0f;
	*/
	float X_world_A = vertCoord_worldSpace_Ground[vertIndex_A].get_X();
	float X_world_B = vertCoord_worldSpace_Ground[vertIndex_B].get_X();
	float X_world_C = vertCoord_worldSpace_Ground[vertIndex_C].get_X();
	float Y_world_A = vertCoord_worldSpace_Ground[vertIndex_A].get_Y();
	float Y_world_B = vertCoord_worldSpace_Ground[vertIndex_B].get_Y();
	float Y_world_C = vertCoord_worldSpace_Ground[vertIndex_C].get_Y();
	float Z_world_A = vertCoord_worldSpace_Ground[vertIndex_A].get_Z();
	float Z_world_B = vertCoord_worldSpace_Ground[vertIndex_B].get_Z();
	float Z_world_C = vertCoord_worldSpace_Ground[vertIndex_C].get_Z();
	/*
	//弱智到气到手抖,超蠢的手误！！！！！！！！
	float X_world_P = X_world_A + (X_world_B - X_world_A)*m + (X_world_C - X_world_C)*n;
	float Y_world_P = Y_world_A + (Y_world_B - Y_world_A)*m + (Y_world_C - Y_world_C)*n;
	float Z_world_P = Z_world_A + (Z_world_B - Z_world_A)*m + (Z_world_C - Z_world_C)*n;
	*/
	float X_world_P = X_world_A + (X_world_B - X_world_A)*m + (X_world_C - X_world_A)*n;
	float Y_world_P = Y_world_A + (Y_world_B - Y_world_A)*m + (Y_world_C - Y_world_A)*n;
	float Z_world_P = Z_world_A + (Z_world_B - Z_world_A)*m + (Z_world_C - Z_world_A)*n;
	Vector4D_float _result(X_world_P, Y_world_P, Z_world_P, 1);
	return(_result);
}